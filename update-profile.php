...
error_reporting(0);
        ...
        if(isset($_POST['submit']))
        {
            ...
            if(requiredInput($userName) &&  requiredInput($email) &&  requiredInput($address) &&  requiredInput($phone)  )
            {
                if(minInput($userName,3))
                {
                    if(validEmail($email))
                    {
                        ...
                        if($password)
                        {
                            ...
                            $sql = "UPDATE `admin` SET  `user_name`='$userName' ,`email`='$email',`password`='$hashed_password', `address`='$address' ,`phone`='$phone'
                            WHERE `id`='{$_SESSION["id"]}' ";

                        }   
                        else 
                        {
                            $sql = "UPDATE `admin` SET  `user_name`='$userName' ,`email`='$email', `address`='$address' ,`phone`='$phone'
                            WHERE `id`='{$_SESSION["id"]}' ";
                        }
